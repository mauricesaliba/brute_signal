...
public class PathLossModel {
    ...
    public double[][] generateHeatMap(AccessPoint[] accessPoints, boolean accumulativeHeatMap) {
        double[][] heatMap = new double[cache.getRows()][cache.getColumns()];
        ...
        for (AccessPoint accessPoint : accessPoints) {
            for (int i = 0; i < cache.getRows(); i++) {
                for (int j = 0; j < cache.getColumns(); j++) {
                    if (i == accessPoint.getCurrentGridPoint().getRow() && j == accessPoint.getCurrentGridPoint().getColumn()) {
                        if (accumulativeHeatMap) {
                    ...
                    double distance = Math.sqrt((Math.pow((accessPoint.getCurrentGridPoint().getRow() - i), 2)) + (Math.pow(accessPoint.getCurrentGridPoint().getColumn() - j, 2)));
                    double totalLoss = findLoss(cache, accessPoint.getCurrentGridPoint().getRow(), accessPoint.getCurrentGridPoint().getColumn(), (i), (j));
    ...
    private class PathLossModelCache {
        ...
        public PathLossModelCache(int rows, int columns) {
            cache = new double[rows][columns];
        }
